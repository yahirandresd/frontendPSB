...
export class AreaPlagasPageComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.programaPlagasId = this.route.snapshot.paramMap.get('programaId') ?? '';
        this.cargarAreas();
    }
    ...
    cargarAreas(): void {
        ...
        this.service.listarPorPrograma(this.programaPlagasId).subscribe({
            next: (data) => { this.areas = data; this.cargando = false; },
            error: () => { this.cargando = false; this.mostrarError('Error al cargar áreas'); }
        });
    }
        ...
        this.confirmationService.confirm({
            ...
            accept: async () => {
                try {
                    ...
                    this.mostrarError('Error al eliminar el área');
                }
            }
        });
    }
